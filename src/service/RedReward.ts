...
export class RedRewardService {
     ...
     */
    async findRedRewardList(uid: string) {
        ...
        if (redEnvelopeList)
            return redEnvelopeList.sort((a, b) => a.isExpired - b.isExpired)
     ...
     * @param rid 领取的红包ID
     */
    async getRedEnvelope(uid: string, rid: string) {
        ...
        redEnvelopeRecordList.forEach(v => {
            ...
            if (v.id === rid) {
                ...
                // 计算角色奖励
                // 计算金币收益
                const startGold = GlobalVar.warbag_man[0];
                const endGold = v.grade * GlobalVar.warbag_man[1];
                let roleGrade = 0
