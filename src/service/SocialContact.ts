...
export class SocialContactService {
	...
	public async armyList(uid: string) {
		...
		const expeditionsList: ArmyList[] = [];
		// 格式化数据
	 ...
	 * @param uid 当前查询用户ID
	 */
	public async expeditionsList(uid: string) {
		// 从Redis拿取数据
		...
		// 拿到玩家上级好友
		const user2 = JSON.parse(await redisDb1.smembers(KeyName.HASH_OBJ_GAME_USERS + user.superiorId).toString());
		...
		logList.forEach(async (v: any) => {
			...
			if (user.friends.indexOf(v.beAttackerId) <= -1) {
				...
				redEnvelopeList.forEach(async (element: any) => {
					element = JSON.parse(element);
					if (element.isDelete === 0 && element.isExpired === 0 && element.isRobbed === 0) {
						// 添加到列表中
						allyList.push(JSON.parse(await redisDb1.smembers(KeyName.HASH_OBJ_GAME_USERS + element.beAttackerId).toString()));
					}
				});
			}
		});
	 ...
	 * @param bid 被攻打玩家ID
	 */
	public async attack(uid: string, beUser: AttackDate) {
		// 判断最近红包
		// 保存攻打记录
		...
		// 保存红包信息
		const redEnvelope = JSON.stringify({
			"id": uuidv4(),
			"grade": 1,
			"isRobbed": 0,
			"isExpired": 0,
