...
import { bullConfig, QUEUE_DATE_FORMAT, QUEUE_USER_ACTIVE, QUEUE_OBJ_NAME } from "../config/BullConfig";
import { KeyName } from "../config/RedisKeys";
import { CltLog } from "../entity/CleLog";
import { TpLog } from "../entity/TpLog";
	...
	public async activeImpl(obj: any) {
		...
		if (exists===0) {
			...
			const userDao = getRepository(User);
                        ...
                        redisDb1.hset(KeyName.HASH_OBJ_GAME_USERS +  user.id, "id",  user.id);
                        redisDb1.hset(KeyName.HASH_OBJ_GAME_USERS +  user.id, "name",  user.name);
                        redisDb1.hset(KeyName.HASH_OBJ_GAME_USERS +  user.id, "passWord", user.passWord);
		}
